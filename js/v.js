...
var lang = getQueryStringValue('lang') || 'en', stretching = getQueryStringValue('stretching') || 'auto';
...
$('select[name=sources]').on('change', function() {
	var _this = $(this), media = _this.closest('.players').find('.media-wrapper').children('div:first').attr('id'), player = mejs.players[media];

	player.setSrc(_this.val().replace('&amp;', '&'));
	player.load();

	var renderer = $('#' + player.media.id + '-rendername');
	renderer.find('.src').html('<a href="' + _this.val() + '" target="_blank">' + _this.val() + '</a>').end().find('.renderer').html(player.media.rendererName).end().find('.error').html('');
});
